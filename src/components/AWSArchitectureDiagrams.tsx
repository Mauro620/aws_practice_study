...
export function AWSArchitectureDiagrams() {
  ...
  return <section className="not-prose rounded-md border border-border bg-background" aria-labelledby="interactive-diagrams-title">
    ...
    <div className="space-y-5 p-5 sm:p-6">
       {mode === 'traffic' && <div className="flex flex-col items-stretch gap-2 lg:flex-row lg:items-center">{items.map((item, index) => <div key={item.id} className="contents"><div className="min-w-0 flex-1"><DiagramNode item={item} selected={selected.id === item.id} onSelect={() => setSelectedId(item.id)} /></div>{index < items.length - 1 && <Arrow />}</div>)}</div>}
       {mode === 'security' && <div className="space-y-3"><div className="flex flex-col items-stretch gap-2 lg:flex-row lg:items-center">{items.map((item, index) => <div key={item.id} className="contents"><div className="min-w-0 flex-1"><DiagramNode item={item} selected={selected.id === item.id} onSelect={() => setSelectedId(item.id)} /></div>{index < items.length - 1 && <Arrow />}</div>)}</div><div className="flex flex-wrap items-center justify-center gap-2 border-t border-dashed border-accent/50 pt-3 text-xs text-accent"><Arrow returnPath /> <span>Respuesta permitida automáticamente, porque el SG es stateful</span> <Arrow /></div><p className="text-sm leading-6 text-foreground/70"><strong className="font-semibold text-foreground">Regla crítica:</strong> 0.0.0.0/0 pertenece solo a los puntos de entrada web públicos, no a SSH ni a puertos de base de datos.</p></div>}
       {mode === 'deployment' && <ol className="grid gap-3 lg:grid-cols-5">{items.map((item, index) => <li key={item.id} className="flex items-stretch gap-2 lg:block"><span className={`flex size-8 shrink-0 items-center justify-center rounded-full border font-mono text-sm ${selected.id === item.id ? 'border-accent bg-accent text-background' : 'border-border text-accent'}`} aria-hidden="true">{index + 1}</span><div className="flex min-w-0 flex-1 items-center lg:mt-2"><DiagramNode item={item} selected={selected.id === item.id} onSelect={() => setSelectedId(item.id)} /></div></li>)}</ol>}
